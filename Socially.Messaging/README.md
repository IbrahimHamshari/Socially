...
To update the database use this command from the Web project folder (replace `Socially.Messaging` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../Socially.Messaging.Infrastructure/Socially.Messaging.Infrastructure.csproj -s Socially.Messaging.Web.csproj
```
