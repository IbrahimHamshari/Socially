...
To update the database use this command from the Web project folder (replace `Socially.SocialGroup` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../Socially.SocialGroup.Infrastructure/Socially.SocialGroup.Infrastructure.csproj -s Socially.SocialGroup.Web.csproj
```
