...
To update the database use this command from the Web project folder (replace `Socially.UserManagment` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../Socially.UserManagment.Infrastructure/Socially.UserManagment.Infrastructure.csproj -s Socially.UserManagment.Web.csproj
```
